...
#https://www.cmegroup.com/markets/agriculture/dairy/class-iii-milk.html#venue=globex

websiteurl = 'https://www.cmegroup.com/markets/agriculture/dairy/class-iii-milk.html#venue=globex'
...
for link in soups.find_all('div', class_ = 'card-body'):
	...
	for price in link.find_all('div', class_ = 'product_link'):
		for ahref in price.find_all('a'):
			#print(ahref.get('href'))
			other_links.append('https://shop.berendbros.com'+ahref.get('href'))
...
for x in other_links:
	...
	soups = soup(webpage,'html.parser')
	#print(soups)
	for alls in soups.find_all('div', class_ = 'breadcrumbs'):
		print(alls.text)

	for description in soups.find_all ('p'):
		print(description.text)

	quant = []
	qa_num = []
	for qa in soups.find_all ('dt', class_ = 'col-md-6 col-5'):
		quant.append(qa.text)
	quant= [x.strip() for x in quant]
	print(quant[9])
	for qanum in soups.find_all ('dd', class_ = 'col-md-6 col-7'):
		#print(qanum.text)
		qa_num.append(qanum.text)
	qa_num= [x.strip() for x in qa_num]
	print(qa_num[8])
'''
'''
websiteurl = 'https://www.mobilesentrix.com/lcd-assembly-with-frame-compatible-for-motorola-moto-g-stylus-5g-xt2315-2023-genuine-oem-rose-champagne'
#print(websiteurl)
try:
    req = Request(websiteurl, headers={"User-Agent": 'Mozilla/5.0'})
    webpage = urlopen(req).read()
    soups = soup(webpage,'html.parser')
    for link in soups.find_all('div', class_ = 'detailpage'):
        for price in link.find_all('div', class_ = 'price-box'):
            print(price.text)
    for link in soups.find_all('h1'):
        print(link.text)
